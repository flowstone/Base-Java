...
CREATE TABLE IF NOT EXISTS `cart` (
  ...
  CONSTRAINT `FK_cart_user` FOREIGN KEY (`uid`) REFERENCES `user` (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='购物车表';

-- 正在导出表  estore.cart 的数据：~0 rows (大约)
/*!40000 ALTER TABLE `cart` DISABLE KEYS */;
/*!40000 ALTER TABLE `cart` ENABLE KEYS */;


-- 导出  表 estore.goods 结构
CREATE TABLE IF NOT EXISTS `goods` (
  ...
  PRIMARY KEY (`id`)
) ENGINE=InnoDB AUTO_INCREMENT=67 DEFAULT CHARSET=utf8 COMMENT='商品表';

-- 正在导出表  estore.goods 的数据：~5 rows (大约)
/*!40000 ALTER TABLE `goods` DISABLE KEYS */;
INSERT INTO `goods` (`id`, `name`, `marketprice`, `estoreprice`, `category`, `num`, `imgurl`, `description`) VALUES
	(1, 'Java编程思想（第4版）', 80.00, 50.00, '图书、电子书刊、音像', 999, '/upload/11/9/f913990ee3f04e029dd34f7d55a60fb1.jpg', '本书赢得了全球程序员的广泛赞誉，即使是最晦涩的概念，在BruceEckel的文字亲和力和小而直接的编程示例面前也会化解于无形。从Java的基础语法到最高级特性（深入的面向对象概念、多线程、自动项目构建、单元测试和调试等），本书都能逐步指导你轻松掌握。'),
	(2, 'JavaScript权威指南（第6版）', 100.00, 80.00, '图书、电子书刊、音像', 999, '/upload/7/13/a9e547531642482b8217669f2698acb4.jpg', '本书要讲述的内容涵盖JavaScript语言本身，以及Web浏览器所实现的JavaScriptAPI。本书第6版涵盖了HTML5和ECMAScript5，很多章节完全重写，增加了当今Web开发的最佳实践的内容，新增的章节包括jQuery、服务器端JavaScript、图形编程以及JavaScript式的面向对象。本书不仅适合初学者系统学习，也适合有经验的 JavaScript 开发者随手翻阅。'),
...
CREATE TABLE IF NOT EXISTS `orderitems` (
  ...
  CONSTRAINT `FK_orderitems_goods` FOREIGN KEY (`gid`) REFERENCES `goods` (`id`),
  CONSTRAINT `FK_orderitems_orders` FOREIGN KEY (`oid`) REFERENCES `orders` (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='订单明细表';

-- 正在导出表  estore.orderitems 的数据：~0 rows (大约)
/*!40000 ALTER TABLE `orderitems` DISABLE KEYS */;
/*!40000 ALTER TABLE `orderitems` ENABLE KEYS */;


-- 导出  表 estore.orders 结构
CREATE TABLE IF NOT EXISTS `orders` (
  ...
  CONSTRAINT `FK_orders_user` FOREIGN KEY (`uid`) REFERENCES `user` (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='订单表';

-- 正在导出表  estore.orders 的数据：~0 rows (大约)
/*!40000 ALTER TABLE `orders` DISABLE KEYS */;
/*!40000 ALTER TABLE `orders` ENABLE KEYS */;


-- 导出  表 estore.province_city_district 结构
CREATE TABLE IF NOT EXISTS `province_city_district` (
...
-- 正在导出表  estore.province_city_district 的数据：~3,522 rows (大约)
/*!40000 ALTER TABLE `province_city_district` DISABLE KEYS */;
INSERT INTO `province_city_district` (`id`, `pid`, `name`) VALUES
	(11, 0, '北京'),
	(12, 0, '天津'),
	(13, 0, '河北'),
	(14, 0, '山西'),
...
CREATE TABLE IF NOT EXISTS `user` (
  ...
  `state` int(1) DEFAULT '-1' COMMENT '激活状态 -1未激活  1 已经激活',
  `code` varchar(64) DEFAULT NULL COMMENT '激活码',
  PRIMARY KEY (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='用户表';

-- 正在导出表  estore.user 的数据：~0 rows (大约)
...
SELECT g.*,sum(oi.buynum) hot FROM orderitems oi, goods g WHERE oi.gid=g.id GROUP BY oi.gid ORDER BY hot DESC LIMIT 0,5;
